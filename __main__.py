...
def generate_agenda_prompt(settings: InputAgenda) -> list[dict[str, str | list[dict[str, str]]]]:
    ...
    user = user_template.substitute(topic=settings.topic, audience=settings.audience, language=settings.language)
    return send_template(developer, user)

@mcp.prompt("Generate Content for a presentation")
def generate_content_prompt(settings: InputAgenda, agenda: OutputAgenda) -> list[
    ...
    developer = developer_template.substitute(style=settings.style)
    user = user_template.substitute(topic=settings.topic, audience=settings.audience, language=settings.language, style=settings.style, header=agenda.title)
    return send_template(developer, user)


# Basic prompt tools
@mcp.tool("Agenda Generation")
def generate_agenda(settings: InputAgenda) -> OutputAgenda:
    agenda_prompt = generate_agenda_prompt(settings)
    response = send_message(agenda_prompt).content
    result = OutputAgenda(agenda=response.split("---"))
    return result

@mcp.tool("Content Generation")
def generate_content(settings: InputAgenda, agenda: List[OutputAgenda]) -> List[Slide]:
    result = []
    for agenda_element in agenda:
        ...
        for paragraph in response.split("---"):
            ...
            slide = Slide(content=paragraph, title=agenda_element.title)
            result.append(slide)
    return result
